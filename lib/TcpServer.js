...
class TcpServer extends net.Server {
  ...
  //send485
  SendClientBind({ mac, type = 485, content, end = "\r" }) {
    let port = this.getClient(mac);
    if (!port) return;
    let client = this.getDevSocket(port);
    if (!client) return;
    ...
    return new Promise((res, rej) => {
      client.event.on("recv", data => {
        console.log(`添加监听器recv++[${client.event.eventNames()}`);

        let len = data.length;
        console.log(data.toString());
        if (type === 485) {
          this.resultArray.push({ mac, data, content, type });
          client.event.off("recv", () =>
            console.log(`rm监听器recv，${type}`)
          );
          res(data);
        } else {
          bufferArr.push(data);
          ...
          if (data.toString().includes(end)) {
            ...
            res(Buffer.concat(bufferArr, bufferArrLength));
          }
        }
      });
      client.socket.write(Buffer.from(type == 485 ? content : content + end));
      setTimeout(() => {
        this.resultArray.push({ mac, content, bufferArr, type: "timeOut" });
        client.event.off("recv", () =>
          console.log(`rm监听器recv，timeOut`)
        );
