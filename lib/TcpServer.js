...
class TcpServer extends net.Server {
  ...
  async GetAllInfo() {
    ...
    let getConnections = util.promisify(this.getConnections);

    let info = {
      local: Object.assign(this.address(), {getConnections:getConnections()}),
      SocketMaps: Object.values(this.SocketMaps).map(
        ({ mac, port, address, jw }) => ({
          mac,
          port,
          address,
      ...
      clients: Object.entries(this.clients),
